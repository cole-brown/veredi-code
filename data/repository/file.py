...
#       load-request: {
#           ...
#           type: DataContext.Type.MONSTERS,
#           campaign: 'forgotten campaign'
#           keys: ['family', 'monster']
#           family: 'dragon',
...
class FileTreeRepository(base.BaseRepository):
    ...
    def _id_type(self,
                 load_type: DataContext.Type,
                 context:   DataContext,
                 out_ids:   List[str]) -> None:
        '''
        Convert the DataContext.Type enum into part of the id usable for getting
        data from this repo.

        Appends the conversion to the `out_ids` list.
        '''
        if load_type == DataContext.Type.PLAYER:
            self._id_accum('players', out_ids)
        elif load_type == DataContext.Type.MONSTER:
            self._id_accum('monsters', out_ids)
        elif load_type == DataContext.Type.NPC:
            self._id_accum('npcs', out_ids)
        elif load_type == DataContext.Type.ITEM:
            self._id_accum('items', out_ids)
        else:
            raise exceptions.LoadError(
                f"No DataContext.Type to ID conversion for: {load_type}",
                None,
                self.context.merge(context))
